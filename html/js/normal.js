    ...
    $.ajax({
        ...
        complete: function(response) {
            if(response.status === 404) {
                document.title = "HoneyDS";
                document.body.hidden = true;
                throw new Error("HoneyDS should be accessed ONLY at its server instance's address (e.g. http://127.0.0.1:1020)");
            }
                ...
                _USER = "";
                document.title = "HoneyDS (unauthorized)";
                setTimeout(function() {
                    $("#login_link").click();
                }, 1000);
            }
...
function init(url, from, to) {
    ...
    document.title = "HoneyDS (loading...)";
    $("body").loader("show");
    $("#main_container").toggleClass("hidden", true);
    $("#heatmap_container").hide();
    $("#calendar_container").hide();
        ...
        complete: function() {
            ...
            PAPAPARSE_COMPLETE_TIMER = setTimeout(function() {
                ...
                for (var threat_text in _THREATS) {
                    ...
                    if(data[LOG_COLUMNS.INFO] == "UNKNOWN_TRAFFIC"){
                        row = [threatUID, data[LOG_COLUMNS.SENSOR], times, severity, minTime, maxTime, sparkline_data.join(","), data[LOG_COLUMNS.SRC_IP], data[LOG_COLUMNS.SRC_PORT], data[LOG_COLUMNS.DST_IP], data[LOG_COLUMNS.DST_PORT], data[LOG_COLUMNS.PROTO], "IP", data[LOG_COLUMNS.TRAIL], data[LOG_COLUMNS.INFO], data[LOG_COLUMNS.REFERENCE], tagData];
                        _DATASET.push(row);
                        }
                }
